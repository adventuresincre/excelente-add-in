...
when-to-use: Load when the parent agent has just spawned a Reviewer subagent on a completed build. Auto-loaded by Work-mode's mandatory pre-summary Reviewer pass.
version: 1.1.0
author: Excelente
---
...
They are the ways a build ships broken while every tool call reported success.

1. **Do the summary formulas point at the right columns?** Pick two aggregates
   and trace their ranges to the header row. A `COUNTIF` over the wrong column
   returns `0`, not an error — in one real build every unit-type count read zero
   because `Type` was in column B and the formulas referenced C.
2. **Is each metric's formula aligned with its own label?** Off-by-one row
   references are endemic in hand-authored summary blocks. `=Q4/Q3` where Q3 is
   a header produces `#VALUE!`; one row further off produces a plausible-looking
   number that is simply wrong. Occupancy at `10800%` is that bug.
3. **Do the magnitudes survive a sanity read?** An average rent of `$0`, a
   negative occupancy, a cap rate of 400%. Arithmetically valid, obviously wrong.
4. **Does anything render as `######` or a clipped label?** Correct numbers in a
   too-narrow column read as a broken model to the person looking at it. Call
   for `autofitColumns`.

Then continue with the full pass below.
...
For sensitivity tables, summary blocks, or any conditional-formatted area, call `screenshot` on the section. Look at the rendered output:
